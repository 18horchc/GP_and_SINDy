...
- Use existing `logistic_growth.m` with `plot_results = false`.
...
- **Irregular (N):** Draw N unique times from U(0, 50), e.g. `sort(rand(N,1)) * 50` or `randperm`-based sampling (PDF says "uniform distribution U(0,50)").
...
| **NLL** | **Negative log (marginal) likelihood** of the GP model. Lower is better. fitrgp does not expose this directly when not using `OptimizeHyperparameters`; when using `'auto'`, the optimization result may report a loss (often negative log likelihood). Implement a helper that computes NLL from the fitted model (kernel + Sigma and training data), or read from `HyperparameterOptimizationResults` when available (see Section 4). |
...
- **Table columns (per run):** TrialTypeId, Kernel, Optimization, NoisePct, Sampling, N, Realization, RMSE, MaxError, **NLL**, PICP, MPIW, CRPS, (optional: runtime_sec).
- Save to **MAT** and/or **CSV** (e.g. `results_gp_logistic_factorial.mat` and `.csv`) so you can use `groupsummary` and `fitlm` in MATLAB.
- **Main effects:** For each factor, average metrics across all other factors; plot factor level vs mean RMSE (and optionally other metrics). For NLL, lower is better.
...
| `gp_logistic_design/fit_and_metrics.m` | Given (t_obs, y_obs), (t_gt, y_gt), kernel, optimization_option → struct of RMSE, MaxError, **NLL**, PICP, MPIW, CRPS. |
...
| `gp_logistic_design/compute_NLL.m` | Compute **negative** log marginal likelihood from fitted GP (if not in model object). |
...
3. **NLL not directly available**  
   fitrgp does not expose the negative log marginal likelihood as a property when using the default fit. When using `'OptimizeHyperparameters','auto'`, the optimization result may contain a loss (often the negative log likelihood).  
   - **Recommendation:** Implement a helper that computes NLL from the fitted model (using the internal representation or by reconstructing the marginal likelihood from kernel parameters and Sigma). Alternatively, read the loss from `HyperparameterOptimizationResults` when using Auto.

4. **Endpoint inclusion for Irregular sampling**  
   PDF says "draw N unique time-steps from U(0, 50)." So 0 and 50 are not guaranteed.  
   - **Recommendation:** Use exactly U(0,50) as stated; if you want to include endpoints for comparability with Regular, document a variant and run it as a separate check.
...
2. Implement metrics: RMSE, MaxError, PICP, MPIW, CRPS (Section 2.4); then **NLL** (helper or from optimization results).  
