...
# Helper class to mimic GPy kernel interface for RBF kernel derivatives
class RBFKernelHelper:
    """Helper class to compute RBF kernel and its derivatives, mimicking GPy interface"""
    def __init__(self, variance=1.0, lengthscale=1.0):
        self.variance = variance  # GPy uses 'variance' (signal variance squared)
        self.lengthscale = lengthscale
        self._variance_array = [variance, lengthscale]  # For indexing compatibility
    
    def __getitem__(self, idx):
        """Allow indexing like GPy kernel: kernel[0] = variance, kernel[1] = lengthscale"""
        return self._variance_array[idx]
    
    def K(self, X, X2=None):
        """Compute RBF kernel matrix K(X, X2) or K(X, X) if X2 is None"""
        if X2 is None:
            X2 = X
        X = X.flatten() if X.ndim > 1 else X
        X2 = X2.flatten() if X2.ndim > 1 else X2
        X1_mat = X[:, np.newaxis]
        X2_mat = X2[np.newaxis, :]
        dist_sq = (X1_mat - X2_mat)**2
        return self.variance * np.exp(-0.5 * dist_sq / self.lengthscale**2)
    
    def dK_dX(self, X, X2, dim=0):
        """Compute derivative of kernel with respect to X: dK/dX
        Returns covariance between d/dX and X2"""
        if X2 is None:
            X2 = X
        X = X.flatten() if X.ndim > 1 else X
        X2 = X2.flatten() if X2.ndim > 1 else X2
        ...
        dist = X1_mat - X2_mat
        dist_sq = dist**2
        K_base = self.variance * np.exp(-0.5 * dist_sq / self.lengthscale**2)
        # dK/dX = -(X - X2) / l^2 * K(X, X2)
        return -(dist / self.lengthscale**2) * K_base
    
    def dK2_dXdX2(self, X, X2, dim1=0, dim2=0):
        """Compute second derivative of kernel: d^2K/(dX dX2)
        Returns covariance between d/dX and d/dX2"""
        if X2 is None:
            X2 = X
        X = X.flatten() if X.ndim > 1 else X
        X2 = X2.flatten() if X2.ndim > 1 else X2
        ...
        dist_sq = dist**2
        K_base = self.variance * np.exp(-0.5 * dist_sq / self.lengthscale**2)
        # d^2K/(dX dX2) = (1/l^2 - (X-X2)^2/l^4) * K(X, X2)
        return (1.0 / self.lengthscale**2 - dist_sq / self.lengthscale**4) * K_base
...
for i in range(0,NumDyn):
    # Create RBF kernel with initial parameters
    # sklearn's RBF uses length_scale parameter (not lengthscale)
    kernel_rbf = RBF(length_scale=1.0, length_scale_bounds=(1e-3, 1e3))
    kernel_white = WhiteKernel(noise_level=1e-6, noise_level_bounds=(1e-10, 1e3))
    kernel = kernel_rbf + kernel_white
    
    # Create and fit GP model
    xtGP = GaussianProcessRegressor(kernel=kernel, alpha=1e-6, n_restarts_optimizer=2)
    xtGP.fit(Xtrain, ytrain[:,i])
    
    # Get optimized kernel parameters
    # Extract RBF kernel (first part of the sum)
    optimized_kernel = xtGP.kernel_
    if hasattr(optimized_kernel, 'k1'):  # If it's a sum kernel (RBF + WhiteKernel)
        rbf_kernel = optimized_kernel.k1
        white_kernel = optimized_kernel.k2
        # Extract length_scale (can be scalar or array, take first element if array)
        lengthscale = rbf_kernel.length_scale
        if np.isscalar(lengthscale):
            lengthscale = float(lengthscale)
        else:
            lengthscale = float(lengthscale[0])
        # scikit-learn's RBF kernel uses signal variance of 1.0 by default
        # (it doesn't expose this as a separate parameter like GPy)
        variance = 1.0
        noise = white_kernel.noise_level
        if np.isscalar(noise):
            noise = float(noise)
        else:
            noise = float(noise[0])
    else:
        # Single RBF kernel (unlikely but handle it)
        lengthscale = optimized_kernel.length_scale
        if np.isscalar(lengthscale):
            lengthscale = float(lengthscale)
        else:
            lengthscale = float(lengthscale[0])
        variance = 1.0
        noise = float(xtGP.alpha) if np.isscalar(xtGP.alpha) else float(xtGP.alpha[0])
    
    # Create helper kernel object for derivative computations
    xtkernel = RBFKernelHelper(variance=variance, lengthscale=lengthscale)
    
    # Predict on training data
    ypred, ystd = xtGP.predict(Xtrain, return_std=True)
    ypred = ypred.reshape(-1, 1)  # Ensure column vector
    yvar = ystd**2  # Convert std to variance
    ...
    GPlist.append({'noise': noise, 'gp_model': xtGP})  # Store noise and model
...
for i in range(0,NumDyn):
    ...
    GPvariance = kernellist[i].variance
    GPlengthscale = kernellist[i].lengthscale
    GPnoise = GPlist[i]['noise']
