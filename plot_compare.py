...
for i in range(len(NoisePer)):
    ...
    for j in range(len(DataSparsity)):
        mean_file = 'result/parameter/Mean_N'+str(NoisePer[i])+'D'+str(DataSparsity[j])+'.npy'
        sg_file = 'result/parameter/SG_N'+str(NoisePer[i])+'D'+str(DataSparsity[j])+'.npy'
        
        if not os.path.exists(mean_file):
            raise FileNotFoundError(
                f"Result file not found: {mean_file}\n"
                f"Please run Scene1.py first to generate the required result files.\n"
                f"Missing file: {mean_file}"
            )
        if not os.path.exists(sg_file):
            raise FileNotFoundError(
                f"Result file not found: {sg_file}\n"
                f"Please run Scene1.py first to generate the required result files.\n"
                f"Missing file: {sg_file}"
            )
        
        gandy_temp = np.load(mean_file).flatten()
        sg_temp = np.load(sg_file).flatten()
