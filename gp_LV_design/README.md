...
**Files:**
- **run_all_LV_experiments.m** — Config-driven driver: runs experiments. Supports `config_filter` and `run_subset`.
...
- **aggregate_LV_metrics.m** — Call separately; compiles all `results_exp_*.mat` into `LV_metrics_all.csv` / `.mat`.

### Config filter examples (run subsets)

```matlab
run_all_LV_experiments(struct('config_filter', @(c) c.noise_pct == 0));  % zero noise only
run_all_LV_experiments(struct('config_filter', @(c) c.is_regular));        % regular sampling only
run_all_LV_experiments(struct('config_filter', @(c) ~c.is_auto));          % default optimization only
```
