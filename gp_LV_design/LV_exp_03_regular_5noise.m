%% GP Lotka-Volterra Experiment 03: Regular sampling, 5% noise, varying N and kernel
%
% Same structure as LV_exp_01 but with 5% Gaussian noise on sampled prey and predator.
...
% 5% noise: sigma = 0.05 * std(ground truth) per state, per design plan
noise_pct = 0.05;
...
%% 3. Loop: each N → sample regularly, 5% noise; each kernel → fit GP for prey and predator, compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 03: Prey — Regular, 5% noise');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 03: Predator — Regular, 5% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 03: Regular sampling, 5% noise');

%% 6. One figure per kernel: 2 rows (prey, predator) x 4 cols (N = 5, 10, 25, 50)
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 03: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        % Row 1: Prey
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP LV Exp 03: %s — Regular, 5%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
save(fullfile(script_dir, 'results_exp_03_regular_5noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
