%% GP Lotka-Volterra Experiment 07: Irregular sampling, 1% noise, varying N and kernel
%
% Same as LV_exp_06 but with 1% Gaussian noise on sampled prey and predator.
% Irregular sampling: N distinct whole-number times in [0, 50], one fixed scheme per N (rng(42)).
%
% Output: Same tables and figures as LV_exp_06 (titles/saves say 07, 1% noise).
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 07: Prey — Irregular, 1% noise');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 07: Predator — Irregular, 1% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 07: Irregular sampling, 1% noise');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 07: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP LV Exp 07: %s — Irregular, 1%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_07_irregular_1noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_07_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_07_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_07_calib_metrics.csv'));
