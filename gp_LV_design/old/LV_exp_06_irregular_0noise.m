%% GP Lotka-Volterra Experiment 06: Irregular sampling, 0% noise, varying N and kernel
%
% Same structure as LV_exp_01 but with IRREGULAR sampling: N distinct whole-number
% time points in [0, 50] chosen once per N (fixed scheme, rng(42)).
%
% Scope:
...
%   - One GP per state (prey and predator); same metrics as LV_exp_01
%
% Output: Three metric tables (Kernel, N, State, ...); RMSE vs N figure; one figure per kernel
%         with 2 rows (prey, predator) x 4 subplots (N=5,10,25,50).
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 06: Prey — Irregular, 0% noise');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 06: Predator — Irregular, 0% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 06: Irregular sampling, 0% noise');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 06: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP LV Exp 06: %s — Irregular, 0%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_06_irregular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_06_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_06_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_06_calib_metrics.csv'));
