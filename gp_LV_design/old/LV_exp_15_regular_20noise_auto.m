%% GP Lotka-Volterra Experiment 15: Regular sampling, 20% noise, Auto hyperparameter optimization
% Same as LV_exp_05 but with OptimizeHyperparameters = 'auto' for built-in kernels. Saves results_exp_15_regular_20noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 12: Prey — Regular, 20% noise, Auto');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 12: Predator — Regular, 20% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 12: Regular sampling, 20% noise, Auto');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 12: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP LV Exp 12: %s — Regular, 1%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_12_regular_20noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
