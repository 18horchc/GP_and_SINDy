%% GP Lotka-Volterra Experiment 09: Irregular sampling, 10% noise
% Same as LV_exp_06 but with 10% Gaussian noise. Irregular: N distinct whole-number times [0,50], rng(42).
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 09: Prey — Irregular, 10% noise');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 09: Predator — Irregular, 10% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 09: Irregular sampling, 10% noise');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 09: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP LV Exp 09: %s — Irregular, 10%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_09_irregular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_09_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_09_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_09_calib_metrics.csv'));
