%% RUN_ALL_LV_EXPERIMENTS  Run all GP Lotka-Volterra experiments and aggregate metrics.
%
...
%   via run_LV_experiment, then aggregate_LV_metrics.
%
...
%     .do_aggregate - run aggregate_LV_metrics at end (default: true).
...
function run_all_LV_experiments(opts)
    ...
    %% Aggregate
    if do_aggregate
        fprintf('Running aggregate_LV_metrics.m\n');
        run(fullfile(script_dir, 'aggregate_LV_metrics.m'));
    end
    fprintf('Done.\n');
end
