...
xlabel('N'); ylabel('RMSE'); title('LV Exp 01: Prey — Regular, 0% noise');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 01: Predator — Regular, 0% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 01: Regular sampling, 0% noise');

%% 6. One figure per kernel: 2 rows (prey, predator) x 4 cols (N = 5, 10, 25, 50)
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 01: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        % Row 1: Prey
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP LV Exp 01: %s — Regular, 0%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
% save(fullfile(script_dir, 'results_exp_01_regular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
