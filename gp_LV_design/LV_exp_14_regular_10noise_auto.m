%% GP Lotka-Volterra Experiment 14: Regular sampling, 10% noise, Auto hyperparameter optimization
% Same as LV_exp_04 but with OptimizeHyperparameters = 'auto' for built-in kernels. Saves results_exp_14_regular_10noise_auto.mat.
clear; clc; close all;
...
for in = 1:length(N_list)
    ...
    t_obs_by_N{in} = t_obs; prey_obs_by_N{in} = prey_obs; pred_obs_by_N{in} = pred_obs;
    ...
    for ik = 1:length(kernel_list)
        ...
        try
            ...
            pred_ymu_prey{ik, in} = ymu_prey; pred_ystd_prey{ik, in} = ystd_prey;
            ...
            pred_ymu_pred{ik, in} = ymu_pred; pred_ystd_pred{ik, in} = ystd_pred;
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 14: Prey — Regular, 10% noise, Auto');
...
xlabel('N'); ylabel('RMSE'); title('LV Exp 14: Predator — Regular, 10% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP LV Exp 14: Regular sampling, 10% noise, Auto');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('LV Exp 14: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP LV Exp 14: %s — Regular, 1%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_12_regular_10noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'prey_gt', 'pred_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_12_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_12_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_12_calib_metrics.csv'));
