...
%
% Loads saved results from LV_exp_01 through LV_exp_05 (0% to 20% noise),
...
%
% Prerequisite: Run LV_exp_01_regular_0noise.m through LV_exp_05_regular_20noise.m
...
for ix = 1:size(exp_files, 1)
    ...
    if ~isfile(fpath)
        warning('Missing %s — run LV_exp_%02d_regular_*noise.m first with Save uncommented.', fname, ix);
        continue
    end
...
if isempty(T_all)
    error('No experiment files found. Run LV_exp_01 through LV_exp_05 with Save uncommented.');
end
