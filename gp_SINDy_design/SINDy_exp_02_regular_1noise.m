%% GP SINDy Experiment 02: Regular sampling, 1% noise, varying N and kernel
%
% Same structure as SINDy_exp_01 but with 1% Gaussian noise on sampled M1, M2.
...
%% 3. Loop: each N → sample regularly, 1% noise; each kernel → fit GP for M1, M2; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 02: M1 — Regular, 1% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 02: M2 — Regular, 1% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 02: Regular sampling, 1% noise');

%% 6. One figure per kernel: 2 rows (M1, M2) x 4 cols (N = 5, 10, 25, 50)
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 02: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP SINDy Exp 02: %s — Regular, 1%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
% save(fullfile(script_dir, 'results_SINDy_exp_02_regular_1noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'M1_gt', 'M2_gt');
% writetable(T_point, fullfile(script_dir, 'results_SINDy_exp_02_point_metrics.csv'));
% writetable(T_prob, fullfile(script_dir, 'results_SINDy_exp_02_prob_metrics.csv'));
% writetable(T_calib, fullfile(script_dir, 'results_SINDy_exp_02_calib_metrics.csv'));
