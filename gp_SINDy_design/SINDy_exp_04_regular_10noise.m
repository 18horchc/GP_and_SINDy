%% GP SINDy Experiment 04: Regular sampling, 10% noise, varying N and kernel
%
% Same structure as SINDy_exp_01 but with 10% Gaussian noise on sampled M1, M2.
% Uses sindy_mcmc_microglia.m defaults (MCMC posterior means, M0=[5;5], tspan=[0 50]).
%
% Scope:
%   - Ground truth: 500 pts on [0, 50] from SINDy+MCMC (M1, M2)
...
%   - One GP per state (M1, M2); same metric tables and plot style as SINDy_exp_01
%
% Output: Three metric tables (Kernel, N, State, ...); RMSE vs N (2 subplots: M1, M2);
%         one figure per kernel with 2 rows (M1, M2) x 4 subplots (N=5,10,25,50): points, ground truth, GP mean + 95%% band.
...
%% 2. Design (5 kernels, same as SINDy_exp_01)
N_list = [5, 10, 25, 50];
...
noise_pct = 0.10;
sigma_noise_M1 = noise_pct * std(M1_gt);
sigma_noise_M2 = noise_pct * std(M2_gt);

rng(42);  % seed for reproducibility

%% 3. Loop: each N → sample regularly, 10% noise; each kernel → fit GP for M1, M2; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
%% 4. Tables (Kernel, N, State with M1 / M2)
...
%% 5. RMSE vs N: two subplots (M1, M2)
figure;
subplot(1, 2, 1);
hold on;
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 04: M1 — Regular, 10% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 04: M2 — Regular, 10% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 04: Regular sampling, 10% noise');

%% 6. One figure per kernel: 2 rows (M1, M2) x 4 cols (N = 5, 10, 25, 50)
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 04: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
        ...
        plot(t_gt, M1_gt, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Ground truth');
        hold on;
        ...
        hold off;
        xlabel('Time'); ylabel('M1 (cells/mm^2)');
        title(sprintf('M1, N = %d', N_list(in)));
        ...
        plot(t_gt, M2_gt, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Ground truth');
        hold on;
        ...
        hold off;
        xlabel('Time'); ylabel('M2 (cells/mm^2)');
        title(sprintf('M2, N = %d', N_list(in)));
        legend('Location', 'best', 'FontSize', 8);
        grid on; xlim([0 50]);
    end
    sgtitle(sprintf('GP SINDy Exp 04: %s — Regular, 10%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
save(fullfile(script_dir, 'results_SINDy_exp_04_regular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'M1_gt', 'M2_gt');
writetable(T_point, fullfile(script_dir, 'results_SINDy_exp_04_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_SINDy_exp_04_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_SINDy_exp_04_calib_metrics.csv'));
