%% RUN_ALL_SINDY_EXPERIMENTS  Run all GP SINDy experiments and aggregate metrics.
%
...
%   via run_SINDy_experiment, then aggregate_SINDy_metrics.
%
...
%     .do_aggregate - run aggregate_SINDy_metrics at end (default: true).
...
function run_all_SINDy_experiments(opts)
    ...
    %% Aggregate
    if do_aggregate
        fprintf('Running aggregate_SINDy_metrics.m\n');
        run(fullfile(script_dir, 'aggregate_SINDy_metrics.m'));
    end
    fprintf('Done.\n');
end
