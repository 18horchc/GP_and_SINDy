%% GP SINDy Experiment 01: Regular sampling, 0% noise, varying N and kernel
%
% Same structure as log_exp_01 but for SINDy+MCMC microglia (two states: M1, M2).
...
%   - Kernels: 5 (Squared Exp, Matern 1/2, 3/2, 5/2, Rational Quadratic) — same as log_exp_01
%   - One GP per state (M1, M2); same metric tables and plot style as log_exp_01
%
...
%% 3. Loop: each N → sample regularly, 0% noise; each kernel → fit GP for M1, M2; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
%% 5. RMSE vs N: two subplots (M1, M2) — M1 and M2 on same figure
figure;
subplot(1, 2, 1);
hold on;
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 01: M1 — Regular, 0% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 01: M2 — Regular, 0% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 01: Regular sampling, 0% noise');

%% 6. One figure per kernel: 2 rows (M1, M2) x 4 cols (N = 5, 10, 25, 50) — M1 and M2 on same figure
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 01: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        % Row 1: M1
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP SINDy Exp 01: %s — Regular, 0%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
save(fullfile(script_dir, 'results_SINDy_exp_01_regular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'M1_gt', 'M2_gt');
writetable(T_point, fullfile(script_dir, 'results_SINDy_exp_01_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_SINDy_exp_01_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_SINDy_exp_01_calib_metrics.csv'));
