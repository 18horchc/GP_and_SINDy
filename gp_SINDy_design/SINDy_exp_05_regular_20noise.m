%% GP SINDy Experiment 05: Regular sampling, 20% noise, varying N and kernel
%
% Same structure as SINDy_exp_01 but with 20% Gaussian noise on sampled M1, M2.
...
%% 3. Loop: each N → sample regularly, 20% noise; each kernel → fit GP for M1, M2; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 05: M1 — Regular, 20% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 05: M2 — Regular, 20% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 05: Regular sampling, 20% noise');

%% 6. One figure per kernel: 2 rows (M1, M2) x 4 cols (N = 5, 10, 25, 50)
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 05: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP SINDy Exp 05: %s — Regular, 20%% noise', kernel_labels{ik}));
end
