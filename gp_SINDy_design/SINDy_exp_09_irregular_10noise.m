%% GP SINDy Experiment 09: Irregular sampling, 10% noise
% Same as SINDy_exp_06 but with 10% Gaussian noise. Irregular: N distinct whole-number times [0,50], rng(42).
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 09: M1 — Irregular, 10% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 09: M2 — Irregular, 10% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 09: Irregular sampling, 10% noise');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 09: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP SINDy Exp 09: %s — Irregular, 10%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_SINDy_exp_09_irregular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'M1_gt', 'M2_gt');
writetable(T_point, fullfile(script_dir, 'results_SINDy_exp_09_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_SINDy_exp_09_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_SINDy_exp_09_calib_metrics.csv'));
