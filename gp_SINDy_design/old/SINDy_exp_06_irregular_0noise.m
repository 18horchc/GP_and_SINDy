%% GP SINDy Experiment 06: Irregular sampling, 0% noise, varying N and kernel
%
% Same structure as SINDy_exp_01 but with IRREGULAR sampling: N distinct whole-number
% time points in [0, 50] chosen once per N (fixed scheme, rng(42)).
%
% Scope: IRREGULAR sampling; 0% noise; N = 5, 10, 25, 50; 5 kernels; one GP per state (M1, M2).
% Output: Same tables and figures as SINDy_exp_01 (titles/saves say 06, Irregular, 0% noise).
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 06: M1 — Irregular, 0% noise');
...
xlabel('N'); ylabel('RMSE'); title('SINDy Exp 06: M2 — Irregular, 0% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
sgtitle('GP SINDy Exp 06: Irregular sampling, 0% noise');

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('SINDy Exp 06: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 4, in);
    ...
    sgtitle(sprintf('GP SINDy Exp 06: %s — Irregular, 0%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_SINDy_exp_06_irregular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'M1_gt', 'M2_gt');
writetable(T_point, fullfile(script_dir, 'results_SINDy_exp_06_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_SINDy_exp_06_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_SINDy_exp_06_calib_metrics.csv'));
