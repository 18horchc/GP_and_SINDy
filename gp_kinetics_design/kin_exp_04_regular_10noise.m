%% GP Kinetics Experiment 04: Regular sampling, 10% noise, varying N and kernel
%
% Same structure as kin_exp_01 but with 10% Gaussian noise on sampled X, Y, Z.
...
%   - Sampling: REGULAR only, 10% Gaussian noise (sigma = 0.10 * std per state)
...
% 10% noise: sigma = 0.10 * std(ground truth) per state
noise_pct = 0.10;
...
%% 3. Loop: each N → sample regularly, 10% noise; each kernel → fit GP for X, Y, Z; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 04: %s — Regular, 10%% noise', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 04: Regular sampling, 10% noise');
...
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 04: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 04: %s — Regular, 10%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
% save(fullfile(script_dir, 'results_kin_exp_04_regular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
% writetable(T_point, fullfile(script_dir, 'results_kin_exp_04_point_metrics.csv'));
% writetable(T_prob, fullfile(script_dir, 'results_kin_exp_04_prob_metrics.csv'));
% writetable(T_calib, fullfile(script_dir, 'results_kin_exp_04_calib_metrics.csv'));
