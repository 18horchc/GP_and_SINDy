...
%   and .csv files matching aggregate_kinetics_metrics expectations. Three states:
%   X, Y, Z (Robertson kinetics). Five built-in kernels only (no periodic).
    ...
    %% Save (kinetics uses results_kin_exp_* naming)
    [mat_fstem, csv_fstem] = get_output_fstems(cfg);
    save(fullfile(out_dir, [mat_fstem '.mat']), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
...
function [mat_fstem, csv_fstem] = get_output_fstems(cfg)
    ...
    mat_fstem = sprintf('results_kin_exp_%02d_%s_%s%s', cfg.exp_id, sam, ns, auto);
    csv_fstem = sprintf('results_kin_exp_%02d', cfg.exp_id);
end
