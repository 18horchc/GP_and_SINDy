%% GP Kinetics Experiment 03: Regular sampling, 5% noise, varying N and kernel
%
% Same structure as kin_exp_01 but with 5% Gaussian noise on sampled X, Y, Z.
...
%% 3. Loop: each N → sample regularly, 5% noise; each kernel → fit GP for X, Y, Z; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 03: %s — Regular, 5%% noise', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 03: Regular sampling, 5% noise');
...
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 03: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 03: %s — Regular, 5%% noise', kernel_labels{ik}));
end
