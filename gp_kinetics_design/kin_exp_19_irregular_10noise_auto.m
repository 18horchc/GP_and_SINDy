%% GP Kinetics Experiment 19: Irregular sampling, 10% noise, Auto hyperparameter optimization
% Same as kin_exp_09 but with OptimizeHyperparameters = 'auto'. Saves results_kin_exp_19_irregular_10noise_auto.mat.
clear; clc; close all;
    ...
    for ik = 1:length(kernel_list)
        ...
        try
            gpr_x = fitrgp(t_obs, x_obs, 'KernelFunction', kern, 'BasisFunction', 'constant', 'FitMethod', 'exact', 'PredictMethod', 'exact', 'Standardize', true, ...
                'OptimizeHyperparameters', 'auto', 'HyperparameterOptimizationOptions', struct('ShowPlots', false, 'Verbose', 0));
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 19: %s — Irregular, 10 pct noise, Auto', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 19: Irregular sampling, 10 pct noise, Auto');
...
colors = {'b', 'g', 'r'};
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 19: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 19: %s — Irregular, 10 pct noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_kin_exp_19_irregular_10noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
writetable(T_point, fullfile(script_dir, 'results_kin_exp_19_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_kin_exp_19_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_kin_exp_19_calib_metrics.csv'));
