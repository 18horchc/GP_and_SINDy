%% GP Kinetics Experiment 01: Regular sampling, 0% noise, varying N and kernel
%
% Same structure as log_exp_01 but for Robertson reaction kinetics (3 states: X, Y, Z).
...
%   - One GP per state (X, Y, Z); same metric tables and plot style as log_exp_01
%
...
%% 2. Design (5 kernels, same as log_exp_01)
N_list = [5, 10, 25, 50];
...
%% 3. Loop: each N → sample regularly, 0% noise; each kernel → fit GP for X, Y, Z; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 01: %s — Regular, 0%% noise', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 01: Regular sampling, 0% noise');
...
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 01: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 01: %s — Regular, 0%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
% save(fullfile(script_dir, 'results_kin_exp_01_regular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
% writetable(T_point, fullfile(script_dir, 'results_kin_exp_01_point_metrics.csv'));
% writetable(T_prob, fullfile(script_dir, 'results_kin_exp_01_prob_metrics.csv'));
% writetable(T_calib, fullfile(script_dir, 'results_kin_exp_01_calib_metrics.csv'));
