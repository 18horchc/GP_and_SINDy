%% GP Kinetics Experiment 06: Irregular sampling, 0% noise, varying N and kernel
%
% Same metrics and plots as kin_exp_01 but with IRREGULAR sampling: N distinct
% whole-number time points in [0, 50] chosen once per N (fixed scheme, rng(42)).
% Uses reaction_kinetics.m defaults (x0=1, y0=0, z0=0, tspan=[0 50]).
%
% Scope:
%   - Ground truth: 500 pts on [0, 50] from Robertson stiff system (x, y, z)
%   - Sampling: IRREGULAR — N distinct integers from 0:50, one scheme per N, 0% noise
%   - Sparsity N: 5, 10, 25, 50
%   - Kernels: 5 (Squared Exp, Matern 1/2, 3/2, 5/2, Rational Quadratic)
%   - One GP per state (X, Y, Z); same metric tables and plot style as kin_exp_01
%
% Output: Three metric tables (Kernel, N, State, ...); RMSE vs N (3 subplots: X, Y, Z);
%         one figure per kernel with 3 rows (X, Y, Z) x 4 subplots (N=5,10,25,50).
...
%% 2. Design (5 kernels, same as kin_exp_01)
N_list = [5, 10, 25, 50];
...
rng(42);  % fixed irregular sampling scheme per N (reproducible); same scheme used in kin_exp_07–10

%% 3. Loop: each N → sample irregularly (whole-number times), 0% noise; each kernel → fit GP; compute metrics
results_point = [];
results_prob = [];
results_calib = [];
pred_ymu_x = cell(length(kernel_list), length(N_list));
pred_ystd_x = cell(length(kernel_list), length(N_list));
pred_ymu_y = cell(length(kernel_list), length(N_list));
pred_ystd_y = cell(length(kernel_list), length(N_list));
pred_ymu_z = cell(length(kernel_list), length(N_list));
pred_ystd_z = cell(length(kernel_list), length(N_list));
t_obs_by_N = cell(1, length(N_list));
x_obs_by_N = cell(1, length(N_list));
y_obs_by_N = cell(1, length(N_list));
z_obs_by_N = cell(1, length(N_list));

for in = 1:length(N_list)
    N = N_list(in);
    % Irregular: N distinct whole-number times in [0, 50] (1 sample per time point)
    ...
    for ik = 1:length(kernel_list)
        ...
        try
            % GP for X
            gpr_x = fitrgp(t_obs, x_obs, 'KernelFunction', kern, 'BasisFunction', 'constant', ...
            ...
            % GP for Y
            gpr_y = fitrgp(t_obs, y_obs, 'KernelFunction', kern, 'BasisFunction', 'constant', ...
                'FitMethod', 'exact', 'PredictMethod', 'exact', 'Standardize', true);
            ...
            % GP for Z
            gpr_z = fitrgp(t_obs, z_obs, 'KernelFunction', kern, 'BasisFunction', 'constant', ...
                'FitMethod', 'exact', 'PredictMethod', 'exact', 'Standardize', true);
...
%% 4. Tables (Kernel, N, State with X / Y / Z)
...
%% 5. RMSE vs N: one subplot per state (X, Y, Z)
figure;
for is = 1:3
    subplot(1, 3, is);
    hold on;
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 06: %s — Irregular, 0%% noise', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 06: Irregular sampling, 0% noise');

%% 6. One figure per kernel: 3 rows (X, Y, Z) x 4 cols (N = 5, 10, 25, 50)
...
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 06: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
            ...
            plot(t_gt, y_gt_s, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Ground truth');
            hold on;
            ...
            hold off;
            xlabel('Time'); ylabel(state_labels{is});
            title(sprintf('%s, N = %d', state_labels{is}, N_list(in)));
            legend('Location', 'best', 'FontSize', 8);
            grid on; xlim([0 50]);
        end
    end
    sgtitle(sprintf('GP Kinetics Exp 06: %s — Irregular, 0%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
save(fullfile(script_dir, 'results_kin_exp_06_irregular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
writetable(T_point, fullfile(script_dir, 'results_kin_exp_06_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_kin_exp_06_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_kin_exp_06_calib_metrics.csv'));
