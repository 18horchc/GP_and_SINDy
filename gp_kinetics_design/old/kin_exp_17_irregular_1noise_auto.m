%% GP Kinetics Experiment 17: Irregular sampling, 1% noise, Auto hyperparameter optimization
% Same as kin_exp_07 but with OptimizeHyperparameters = 'auto'. Saves results_kin_exp_17_irregular_1noise_auto.mat.
clear; clc; close all;
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 17: %s — Irregular, 1 pct noise, Auto', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 17: Irregular sampling, 1 pct noise, Auto');
...
colors = {'b', 'g', 'r'};
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 17: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 17: %s — Irregular, 1 pct noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_kin_exp_17_irregular_1noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
writetable(T_point, fullfile(script_dir, 'results_kin_exp_17_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_kin_exp_17_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_kin_exp_17_calib_metrics.csv'));
