%% GP Kinetics Experiment 09: Irregular sampling, 10% noise, varying N and kernel
%
% Same as kin_exp_06 but with 10% Gaussian noise on sampled X, Y, Z.
% Irregular sampling: N distinct whole-number times in [0, 50], one fixed scheme per N (rng(42)).
%
% Scope: IRREGULAR sampling; 10% noise (sigma = 0.10*std per state); N = 5, 10, 25, 50; 5 kernels; one GP per state (X, Y, Z).
% Output: Same tables and figures as kin_exp_06 (titles/saves say 09, 10% noise).
...
for is = 1:3
    ...
    xlabel('N'); ylabel('RMSE'); title(sprintf('Kin Exp 09: %s — Irregular, 10%% noise', state_labels{is}));
    legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);
end
sgtitle('GP Kinetics Exp 09: Irregular sampling, 10% noise');
...
colors = {'b', 'g', 'r'};
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Kin Exp 09: %s', kernel_labels{ik}));
    for is = 1:3
        y_gt_s = gt_cell{is};
        for in = 1:length(N_list)
    ...
    sgtitle(sprintf('GP Kinetics Exp 09: %s — Irregular, 10%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_kin_exp_09_irregular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'x_gt', 'y_gt', 'z_gt');
writetable(T_point, fullfile(script_dir, 'results_kin_exp_09_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_kin_exp_09_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_kin_exp_09_calib_metrics.csv'));
