%% GP Logistic Experiment 12: Regular sampling, 1% noise, Auto hyperparameter optimization
% Same as log_exp_02 but with OptimizeHyperparameters = 'auto'. Saves results_exp_12_regular_1noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 12: Regular, 1% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 12: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 12: %s — Regular, 1%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_12_regular_1noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
