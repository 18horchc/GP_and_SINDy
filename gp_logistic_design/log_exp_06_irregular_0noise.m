%% GP Logistic Experiment 06: Irregular sampling, 0% noise, varying N and kernel
%
% Same structure as log_exp_01 but with IRREGULAR sampling: N distinct whole-number
% time points in [0, 50] chosen once per N (fixed scheme, rng(42)).
%
% Scope: IRREGULAR sampling; 0% noise; N = 5, 10, 25, 50; 5 kernels.
% Output: Same tables and figures as log_exp_01 (titles/saves say 06, Irregular, 0% noise).
...
for in = 1:length(N_list)
    ...
    for ik = 1:length(kernel_list)
        kern = kernel_list{ik};
        try
            gpr = fitrgp(t_obs, y_obs, 'KernelFunction', kern, 'BasisFunction', 'constant', ...
...
xlabel('Number of points (N)'); ylabel('RMSE');
title('GP Logistic Exp 06: Irregular sampling, 0% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 06: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 06: %s — Irregular sampling, 0%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_06_irregular_0noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
