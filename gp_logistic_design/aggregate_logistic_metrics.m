...
%
% Loads saved results from exp 01-60 (20 base + 20 with 3 reps + 20 with 8 reps).
...
%         Columns: Noise, N, N_replicates, Kernel, Regular, Optimization,
...
% Build file list from same config pattern as run_all_logistic_experiments
...
for ix = 1:size(exp_files, 1)
    ...
    % Backward compatibility: add N_replicates if missing (old 01-20 format)
    ...
    join_keys = {'Kernel', 'N', 'N_replicates'};
    ...
    T_merged = T_merged(:, {'Noise', 'N', 'N_replicates', 'Kernel', 'Regular', 'Optimization', 'N_per_Time', 'RMSE', 'MAE', 'R2', 'NLPD', 'MSLL', 'CRPS', 'sMSE', 'Coverage', 'NLML'});
...
function fname = get_fname(exp_id, noise, is_regular, is_auto, n_rep)
    ...
    fname = sprintf('results_exp_%02d_%s_%s%s%s.mat', exp_id, sam, ns, auto, rep_suf);
end
