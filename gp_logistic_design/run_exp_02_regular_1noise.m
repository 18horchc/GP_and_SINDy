%% GP Logistic Experiment 02: Regular sampling, 1% noise, varying N and kernel
%
...
%   - Noise: 1% Gaussian (sigma = 0.01 * std(y_gt))
...
% 1% noise: sigma = 0.01 * std(ground truth), per design plan
noise_pct = 0.01;
sigma_noise = noise_pct * std(y_gt);

%% 3. Loop: for each N, sample regularly with 1% noise; for each kernel, fit GP and compute all metrics
...
ylabel('RMSE');
title('GP Logistic Exp 02: Regular sampling, 1% noise');
...
for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 02: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
        N = N_list(in);
        t_obs = t_obs_by_N{in};
    ...
    sgtitle(sprintf('GP Logistic Exp 02: %s — Regular sampling, 1%% noise', kernel_labels{ik}));
end

%% 7. Save (optional)
% save(fullfile(script_dir, 'results_exp_02_regular_1noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
