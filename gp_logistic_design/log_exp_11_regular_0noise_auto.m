%% GP Logistic Experiment 11: Regular sampling, 0% noise, Auto hyperparameter optimization
%
% Same as log_exp_01 but with OptimizeHyperparameters = 'auto' (ShowPlots false, Verbose 0).
% All other options unchanged: KernelFunction, BasisFunction, FitMethod, PredictMethod, Standardize.
%
% Output: Same tables and figures; saves to results_exp_11_regular_0noise_auto.mat and *_metrics.csv.
...
title('GP Logistic Exp 11: Regular, 0% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 11: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 11: %s — Regular, 0%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_11_regular_0noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
