%% GP Logistic Experiment 15: Regular sampling, 20% noise, Auto hyperparameter optimization
% Same as log_exp_05 but with OptimizeHyperparameters = 'auto'. Saves results_exp_15_regular_20noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 15: Regular, 20% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 15: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 15: %s — Regular, 20%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_15_regular_20noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_15_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_15_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_15_calib_metrics.csv'));
