%% GP Logistic Experiment 13: Regular sampling, 5% noise, Auto hyperparameter optimization
% Same as log_exp_03 but with OptimizeHyperparameters = 'auto'. Saves results_exp_13_regular_5noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 13: Regular, 5% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 13: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 13: %s — Regular, 5%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_13_regular_5noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_13_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_13_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_13_calib_metrics.csv'));
