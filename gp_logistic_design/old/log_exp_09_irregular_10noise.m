%% GP Logistic Experiment 09: Irregular sampling, 10% noise
% Same as log_exp_06 but with 10% Gaussian noise. Irregular: N distinct whole-number times [0,50], rng(42).
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 09: Irregular sampling, 10% noise');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 09: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 09: %s — Irregular, 10%% noise', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_09_irregular_10noise.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
