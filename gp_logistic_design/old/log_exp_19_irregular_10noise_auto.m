%% GP Logistic Experiment 19: Irregular sampling, 10% noise, Auto hyperparameter optimization
% Same as log_exp_09 but with OptimizeHyperparameters = 'auto'. Saves results_exp_19_irregular_10noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 19: Irregular, 10% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 19: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 19: %s — Irregular, 10%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_19_irregular_10noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_19_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_19_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_19_calib_metrics.csv'));
