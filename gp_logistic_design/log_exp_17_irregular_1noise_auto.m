%% GP Logistic Experiment 17: Irregular sampling, 1% noise, Auto hyperparameter optimization
% Same as log_exp_07 but with OptimizeHyperparameters = 'auto'. Saves results_exp_17_irregular_1noise_auto.mat.
clear; clc; close all;
...
xlabel('N'); ylabel('RMSE'); title('GP Logistic Exp 17: Irregular, 1% noise, Auto');
legend('Location', 'best'); grid on; set(gca, 'XTick', N_list);

for ik = 1:length(kernel_labels)
    figure('Name', sprintf('Exp 17: %s', kernel_labels{ik}));
    for in = 1:length(N_list)
        subplot(2, 2, in);
    ...
    sgtitle(sprintf('GP Logistic Exp 17: %s — Irregular, 1%% noise, Auto', kernel_labels{ik}));
end

save(fullfile(script_dir, 'results_exp_17_irregular_1noise_auto.mat'), 'T_point', 'T_prob', 'T_calib', 't_gt', 'y_gt');
writetable(T_point, fullfile(script_dir, 'results_exp_17_point_metrics.csv'));
writetable(T_prob, fullfile(script_dir, 'results_exp_17_prob_metrics.csv'));
writetable(T_calib, fullfile(script_dir, 'results_exp_17_calib_metrics.csv'));
